...
```yaml
# audit: p0.5
file: rules/rag/search-rerank/neural-rerankers/CLAUDE.md
date: 2026-05-26
auditor: rocklambros
status: failed
checks:
  api_currency:
    status: partial
    notes: >
      cohere.Client() is legacy SDK v4; SDK v5 uses cohere.ClientV2.
      sentence-transformers CrossEncoder not instantiated in Do examples (validation
      class only). bge-reranker absent from all allowlists.
  owasp_2025_refs:
    status: fail
    notes: >
      All OWASP refs use bare "OWASP LLM01" with no ":2025" qualifier.
      Applies to every rule in the file.
  pii_redaction_before_vendor_rerank:
    status: fail
    notes: >
      Cohere rule ships raw document text to external API with no PII scrubbing
      or field-level redaction step before the network call.
  model_integrity_safetensors:
    status: fail
    notes: >
      Jina rule calls AutoModelForSequenceClassification.from_pretrained without
      use_safetensors=True. ColBERT checksums .pt files but does not enforce
      safetensors format.
  adversarial_query_doc_manipulation:
    status: partial
    notes: >
      CrossEncoderValidator detects prompt-injection markers but does not address
      Unicode homoglyphs, invisible characters, or gradient-based adversarial
      perturbations designed to flip relevance scores.
  latency_budget_vs_dos:
    status: fail
    notes: >
      No inference timeout on local model calls (Jina, FlashRank, ColBERT).
      No per-request SLA ceiling. Cohere rule has per-user rate limiting but no
      request-level timeout to bound tail latency under adversarial load.
defects:
  - id: D1
    severity: warning
    description: cohere.Client() is SDK v4; migrate to cohere.ClientV2 for SDK v5 compatibility.
  - id: D2
    severity: strict
    description: All OWASP refs must carry ":2025" qualifier (e.g. "OWASP LLM01:2025").
  - id: D3
    severity: strict
    description: PII redaction required before documents are sent to Cohere API.
  - id: D4
    severity: warning
    description: Add use_safetensors=True to AutoModelForSequenceClassification.from_pretrained in Jina rule.
  - id: D5
    severity: warning
    description: Add inference timeout (e.g. threading/asyncio timeout wrapper) to local model rerank calls.
  - id: D6
    severity: advisory
    description: Expand adversarial-input detection to cover Unicode homoglyphs and invisible characters.
```
