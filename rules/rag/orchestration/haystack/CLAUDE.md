...
```yaml
- file: rules/rag/orchestration/haystack/CLAUDE.md
  audit: p0.5
  date: 2026-05-26
  status: failed
  defects:
    - id: D1
      ...
      detail: All OWASP refs use bare names (e.g., "OWASP LLM01 (Prompt Injection)") with no :2025 suffix. Neither OWASP Top 10:2025 nor OWASP LLM Top 10:2025 appears anywhere in the file.
    - id: D2
      check: component-tool-agent-boundary
      detail: No rule addresses ComponentTool and agent tool-use boundaries. Haystack 2.x Agent with ComponentTool wraps arbitrary pipeline components as callable tools, creating a distinct trust boundary where a compromised or untrusted component gains tool-call authority inside an agent loop.
    - id: D3
      check: promptbuilder-jinja-injection
      detail: PromptBuilder is used in examples but no rule explicitly covers Jinja2 template injection risk. User-controlled template strings passed to PromptBuilder allow {{ 7*7 }} or {% for %} constructs to render arbitrary expressions; the rule should mandate static templates and reject dynamic template strings from untrusted input.
    - id: D4
      check: secret-from-env-var
      detail: All credential examples use os.environ.get() directly rather than Haystack's Secret.from_env_var() API introduced in Haystack 2.x. Secret.from_env_var() integrates with Haystack's secret management layer and prevents accidental serialization of credentials in pipeline YAML exports; the file should demonstrate this pattern.
```
